...
/**
 * COEUS API handler class. <p>Main API class for internal Java usage, use this
 * or internal when building new data access methods!</p> <p> <ul>
 * <li>select*|get* are data extraction methods for SPARQL queries and direct
 * triplets.</li> <li>add*|create* are data input methods for data
 * creating.</li> </ul> </p>
 *
 * @author pedrolopes
 */
public class API {
    ...
    /**
     * Get the first name for a given OMIM code.
     * @param omim OMIM accession number
     * @return 
     */
    public String getOmimName(String omim) {
        try {
            return getTriple("diseasecard:omim_" + omim, "diseasecard:name", "o", "csv").split("\n")[1];
        } catch (Exception e) {
            return "";
        }
    }
